...
class LeWMRuntime:
    ...
    async def encode(self, request: RequestEnvelope) -> ResponseEnvelope:
        ...
        return ResponseEnvelope(
            request_id=request.request_id, operation="encode", model=self.model_id, model_revision=self.revision,
    ...
    async def rollout(self, request: RequestEnvelope) -> ResponseEnvelope:
        ...
        start = time.perf_counter()
        with telemetry.span("wmcp.model.rollout", **{"wmcp.candidate_count": s, "wmcp.horizon": t}), torch.inference_mode():
        ...
        return ResponseEnvelope(
            request_id=request.request_id, operation="rollout", model=self.model_id, model_revision=self.revision,
            outputs={"predicted_latents": self._tensor_output(request.request_id, "predicted_latents", predicted, "B,S,T,D")},
    ...
    async def score(self, request: RequestEnvelope) -> ResponseEnvelope:
        ...
        return ResponseEnvelope(
            request_id=request.request_id, operation="score", model=self.model_id, model_revision=self.revision,
            outputs={
                "costs": self._tensor_output(request.request_id, "costs", costs, "B,S"),
                "best_index": [int(i) for i in best_index],
                "cost_statistics": {
                    "min": float(costs.min()), "mean": float(costs.mean()), "max": float(costs.max()),
                },
            },
            diagnostics={"backend": self.backend, "candidate_count": s, "horizon": t},
        )
    ...
    async def plan(self, request: RequestEnvelope) -> ResponseEnvelope:
        ...
        return ResponseEnvelope(
            request_id=request.request_id, operation="plan", model=self.model_id, model_revision=self.revision,
            outputs={
                "best_action_sequence": self._tensor_output(request.request_id, "plan", seq, "B,T,A"),
                "first_action": {"kind": "tensor", "encoding": "inline", "dtype": "float32",
                                 "shape": [1, action_dim], "layout": "B,A", "data": [seq[0, 0].tolist()]},
                "best_cost": [best_cost],
                "planner_diagnostics": {
                    "iterations": iterations, "candidates": population,
